...
```{r}
cor(sermons$price, sermons$ramadan)
```
...
```{r}
fit30 <- estimateEffect(~s(date) + casualty + price + ramadan + location , 
...
```{r}
plot(fit30, "date", method = "continuous", topics = c(20), ci.level = 0.95,
     model = stm30, printlegend = TRUE, xaxt = "n", xlab = "Time (2001-2020)", 
...
```
```{r}
plot(fit30, "casualty", method = "continuous", topics = c(20), ci.level = 0.95,
