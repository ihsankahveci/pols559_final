...
```{r, message=FALSE}
sermons = read_csv("sermons_final.csv") %>% drop_na()
sermons %>% summary 
```
...
stm_models = manyTopics(out$documents, out$vocab, data = out$meta,
                       K = 10:15, 
                       prevalence =  ~s(date) + word_count + price + ramadan + sacrifice, 
...
stm12 = stm(docs, vocab, data = meta,
            K = 12,  
            prevalence = ~s(date) + casualty + price + ramadan + location,  
            seed = 57,
            verbose =  F)

par(cex=0.6)
plot(stm12,  n=7)
topicQuality(stm12, documents = out$documents)
```

```{r}
toLDAvis(stm12, docs = docs)
```
...
```{r}
fit12 <- estimateEffect(~s(date) + casualty + price + ramadan + location , 
                         stm12,  meta = out$meta, uncertainty = "Global")

```


```{r}
plot(fit12, "date", method = "continuous", topics = c(7), ci.level = 0.95,
     model = stm12, printlegend = TRUE, xaxt = "n", xlab = "Time (2008)", 
...
```
```{r}
plot(fit12, "price", method = "continuous", topics = c(8), ci.level = 0.9,
     model = stm12, printlegend = TRUE, xaxt = "n", xlab = "price", 
     labeltype = "custom", custom.labels = c("trade"))
```
```{r}
plot(fit12, "casualty", method = "continuous", topics = c(7), ci.level = 0.9,
     model = stm12, printlegend = TRUE, xaxt = "n", xlab = "casualty", 
     labeltype = "custom", custom.labels = c("nationalism"))
```
...
model = stm12, printlegend = TRUE, cov.value1 = 1, cov.value2 = 0)
```

```{r}
par(cex=0.6)
plot(fit12, "location", method = "difference", topics = c(2,3,5,6),
model = stm12, printlegend = TRUE, cov.value1 = "ankara", cov.value2 = "istanbul")
```
